...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function fetchAndDisplayData() {
        ...
        .then(data => {
            ...
            Object.entries(jobTabs).forEach(([tab, jobType]) => {
                const tableBody = document.querySelector(`#sortableTable${tab} .list`);
                ...
                let sortedData = filteredData.sort((a, b) => parseInt(a.totalMinutes) - parseInt(b.totalMinutes));
    ...
    function addToQueue(item, jobType) {
        ...
        const formattedTimeTillShiftEnds = formatMinutesToHours(minutesTillEndOfShift);
        ...
        document.querySelectorAll(`tr[data-user-key="${item.name}"]`).forEach(el => el.remove());
    }
    

    // Additional handlers and utility functions
    document.getElementById('resetQueueButton').addEventListener('click', function() {
        window.location.reload();  // Refreshes the page
    });

    document.getElementById('submitQueueButton').addEventListener('click', function() {
        const queueTableBody = document.querySelector('#queueTableBody');
        const users = [];
        ...
        queueTableBody.querySelectorAll('tr').forEach(row => {
            ...
            console.log(`Time till end of shift for ${name}: ${formatMinutesToHours(minutesTillEndOfShift)}`);  // Logging the time till end of shift
            users.push({
                name: name,
                jobType: jobType,
